...
#define CONFIGURE_MAXIMUM_PERIODS             0
   ...
   /* if the application does not define if the task blockage/suspension is secure
    * when task has obtained a semaphore with priority inheritance/ceiling then use
    * the default RTEMS behavior (not secure) */
#ifndef CONFIGURE_SEM_SECURE_BLOCK_SUSPEND

   /* then define it to 0 */
#define CONFIGURE_SEM_SECURE_BLOCK_SUSPEND    0
...
(( CONFIGURE_MEMORY_FOR_TASKS(CONFIGURE_MAXIMUM_TASKS) + /* tasks */                            \
   ...
   rtems_api_configuration_table Configuration_RTEMS_API = {
                                                            ...
                                                            CONFIGURE_SEM_SECURE_BLOCK_SUSPEND
   };
