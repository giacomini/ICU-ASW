 ...
 *  $Rev: 12302 $ | $Date: 2013-12-04 11:25:23 +0000 (Wed, 04 Dec 2013) $| $Author: hsilva $ | SPR 3137
 *
 **/
                                                        ...
                                                        )
{
   ...
   if(mutex != NULL && _CORE_mutex_Is_inherit_priority(&mutex->Attributes))
   {
      ...
      /* reset the node */
      _reset_Chain_Node(&the_thread->Wait.node);
      ...
      if(!_Chain_Is_empty(&holder->Wait.threads_blocked))
      {
         ...
         Thread_Wait_information *wait_info = (Thread_Wait_information *) 
            ((Chain_Node * ) _Chain_Tail(&holder->Wait.threads_blocked))->previous;
