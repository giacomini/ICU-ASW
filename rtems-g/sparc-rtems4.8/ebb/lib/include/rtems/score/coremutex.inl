 ...
 *  11818   | 07/05/2013  | hsilva       | SPR 3138
 *  $Rev: 12190 $ | $Date: 2013-11-11 17:57:57 +0000 (Mon, 11 Nov 2013) $| $Author: hsilva $ | SW-FMECA-19, 20, 21 and 24
 *
 **/
...
/**
 *  @brief determine if Mutex is simple binary semaphore
 *
 *  This routine returns TRUE if the mutex's is
 *  INHERIT_PRIORITY and FALSE otherwise.
 *
 *  @param[in] the_attribute is the attribute set of the mutex
 *
 *  @return returns TRUE if the mutex is simple binary.
 */
RTEMS_INLINE_ROUTINE boolean _CORE_mutex_Is_simple_binary(
                                                          CORE_mutex_Attributes *the_attribute
                                                          )
{
	/* return true if the mutex attribute discipline is core mutex nesting blocks */
	   return the_attribute->lock_nesting_behavior == CORE_MUTEX_NESTING_BLOCKS;
}
 ...
 *  ceiling.
 */
RTEMS_INLINE_ROUTINE boolean _CORE_mutex_Is_ceiling_or_inherit(
                                                             CORE_mutex_Attributes *the_attribute
                                                             )
{
   /* return true if the mutex attribute discipline is ceiling */
   return (the_attribute->discipline == CORE_MUTEX_DISCIPLINES_PRIORITY_CEILING ||
           the_attribute->discipline == CORE_MUTEX_DISCIPLINES_PRIORITY_INHERIT);
}
                                            ...
                                            ISR_Level level)
{
    ...
    rtems_boolean isInheritorCeiling = FALSE;
    Chain_Control *hold_mutexes = NULL;
   ...
   isInheritorCeiling = _CORE_mutex_Is_ceiling_or_inherit(&the_mutex->Attributes);
    
   if(!_Thread_Executing->is_preemptible && isInheritorCeiling)
   {
       /* mutexes priority disciplines has been violated, cannot seize the mutex */
       _Thread_Executing->Wait.return_code = CORE_MUTEX_MUTEX_UNSATISFIED_WAIT;
       _ISR_Enable(level);
       return;
   }
   
    /* verify if the task has any semaphore with
     * priority ceiling or priority inheritance
     * SW-FMECA-20, the thread field resource_count
     * has the number of priority ceilings/inheritances
     * hold/obtained by thread
     */
    hold_mutexes = &_Thread_Executing->Wait.mutexes_hold;
    if(isInheritorCeiling && !_Chain_Is_empty(hold_mutexes)) {
        Chain_Node *the_node = _Chain_Head(hold_mutexes);
        CORE_mutex_Control *cmxc = NULL;
        while (!_Chain_Is_tail(hold_mutexes, the_node)) {
            cmxc = (CORE_mutex_Control*) the_node;
            if (
                    _CORE_mutex_Is_ceiling_or_inherit(&cmxc->Attributes) &&
                    (cmxc->Attributes.discipline != the_mutex->Attributes.discipline &&
                    cmxc->holder == _Thread_Executing
                    )
                    ) {
                /* mutexes priority disciplines has been violated, cannot seize the mutex */
