 ...
 *  $Rev: 10173 $ | $Date: 2011-05-18 18:31:44 +0100 (Wed, 18 May 2011) $| $Author: hsilva $ | SPR 2967 - Questionable no return attribute for rtems_shutdown_executive()
 *
 **/
...
/**
 *  @brief determine if the system state is in application safe state
 *  
 *  This function returns TRUE if the state is equal to the
 *  SYSTEM_APP_SAFE state, and FALSE otherwise.
 *
 *  @param[in] state the state to check
 *
 *  @return returns TRUE if the system state is SYSTEM_APP_SAFE and FALSE otherwise.
 */
RTEMS_INLINE_ROUTINE boolean _System_state_Is_safeState(
                                                 System_state_Codes state
                                                 )
{
   /* system is up if is equal to SYSTEM_STATE_APP_SAFE */
   return (state == SYSTEM_STATE_APP_SAFE );
}
